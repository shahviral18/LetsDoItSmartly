...
export default function EmailSurveillancePage() {
  ...
  const selectedDomain = allDomains.find(d => d.name === domain);
  const domainUsers = mockUsers.filter(u => u.domain === domain);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newReq: BccRequest = {
      id: `bcc${Date.now()}`,
      domain,
      ouPath: selectedDomain?.ou ?? "",
      billingEntityId: selectedDomain?.billingEntityId ?? "",
      affectedUsers: affectedType === "all" ? "all" : specificUsers,
      surveillanceEmail,
      directions,
      status: "pending",
      requestedBy: user?.id ?? "",
  ...
  const myRequests = isDomainOwner
    ? requests.filter(r => r.requestedBy === user?.id || ["abc.com","abctech.com"].includes(r.domain))
    : requests;
    ...
    <div className="p-6 space-y-6 animate-fade-in">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                {selectedDomain && (
                  <p className="text-[11px] text-muted-foreground">OU Path: <span className="font-mono text-foreground">{selectedDomain.ou}</span></p>
                )}
              </div>
      ...
      <div className="bg-card rounded-xl border border-border shadow-card overflow-hidden">
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          ...
          <span className="text-xs text-muted-foreground">{myRequests.length} request{myRequests.length !== 1 ? "s" : ""}</span>
        </div>
        {myRequests.length === 0 ? (
              ...
              <motion.div key={req.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.04 }}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground truncate">
                    BCC → <span className="font-medium text-foreground">{req.surveillanceEmail}</span>
                    {" · "}
                    {req.directions.map(d => directionLabels[d].label).join(", ")}
                    {" · "}
                    {req.affectedUsers === "all" ? "All users" : `${(req.affectedUsers as string[]).length} user(s)`}
                  </p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xs text-muted-foreground">{new Date(req.requestedAt).toLocaleDateString("en-IN")}</p>
                  {req.completedAt && <p className="text-[10px] text-success">Done {new Date(req.completedAt).toLocaleDateString("en-IN")}</p>}
                </div>
